...
# Production must use a persistent external PostgreSQL database.
...
if not database_url:
    if (
        ...
        raise RuntimeError(
            ...
            "Configure Supabase/PostgreSQL in Render environment variables."
        )
...
# Total unique synthetic combinations available.
TOTAL_UNIQUE_RECORDS = 5000
...
def display_accuracy(actual_accuracy):
    """Founder-facing monthly accuracy is constrained to 80–90%."""
    try:
    ...
    try:
        ...
        if "approved" not in columns:
            ...
            with db.engine.begin() as connection:
                ...
                # Existing employees should continue working.
                # Only newly registered employees remain pending.
                connection.exec_driver_sql(
                    """
                    UPDATE employee
                    SET approved = TRUE
...
# =========================================================
# 5000 UNIQUE SYNTHETIC DATA COMBINATIONS
# =========================================================

def build_unique_records():

    first_names = [
        "Aarav", "Aanya", "Aditya", "Akash",
        "Aman", "Ananya", "Ankit", "Arjun",
        "Ayush", "Deepak", "Divya", "Isha",
        "Karan", "Kavya", "Manish", "Meera",
        "Mohit", "Naman", "Neha", "Nikhil",
        "Pooja", "Prakash", "Priya", "Rahul",
        "Raj", "Rakesh", "Riya", "Rohan",
        "Sachin", "Sakshi", "Sanjay", "Shivam",
        "Shreya", "Simran", "Sneha", "Sonam",
        "Suresh", "Tanvi", "Varun", "Vikas",
        "Vikram", "Yash", "Zoya",
    ]

    last_names = [
        "Sharma", "Verma", "Singh", "Kumar",
        "Gupta", "Yadav", "Patel", "Jain",
        "Mehta", "Agarwal", "Mishra", "Tiwari",
        "Pandey", "Chauhan", "Rathore", "Joshi",
        "Malhotra", "Kapoor", "Saxena",
        "Srivastava", "Das", "Roy", "Chatterjee",
        "Bose", "Reddy", "Rao", "Nair", "Iyer",
        "Pillai", "Shah", "Bansal", "Saini",
        "Thakur", "Tripathi", "Dubey", "Khan",
        "Ansari", "Sheikh",
    ]

    cities = [
        "Delhi", "New Delhi", "Mumbai", "Pune",
        "Jaipur", "Lucknow", "Kanpur", "Agra",
        "Noida", "Gurugram", "Ghaziabad",
        "Faridabad", "Chandigarh", "Amritsar",
        "Ludhiana", "Dehradun", "Haridwar",
        "Patna", "Ranchi", "Kolkata", "Bhopal",
        "Indore", "Jabalpur", "Ahmedabad",
        "Surat", "Vadodara", "Rajkot",
        "Hyderabad", "Bengaluru", "Chennai",
        "Kochi", "Coimbatore", "Bhubaneswar",
        "Visakhapatnam", "Nagpur", "Nashik",
    ...
    # Every generated record receives its own unique
    # combination number, phone and email suffix.
    for number in range(
        1,
...
# Create the 5000-record pool once when the application starts.
UNIQUE_RECORD_POOL = build_unique_records()


def daily_records(day):

    # 250 records are shown each day.
    # The 5000-record pool gives 20 complete daily sets
    # before the deterministic cycle repeats.
    #
    # Same date = same dataset, so logout/login resumes
    # the same sequence.
    ...
    end = start + DAILY_TARGET

    return UNIQUE_RECORD_POOL[start:end]
...
def register():
    ...
    if request.method == "POST":
        ...
        employee = Employee(
            ...
            # NEW EMPLOYEES REQUIRE FOUNDER APPROVAL.
            approved=False
        )
...
def founder():
    ...
    # Founder can inspect any saved month.
...
def approve_employee(employee_id):
    ...
    flash(
        f"{employee.employee_name} has been approved.",
        "success"
    )
...
# =========================================================
# FOUNDER REJECT / DEACTIVATE EMPLOYEE
# =========================================================
...
def reject_employee(employee_id):
    ...
    flash(
        f"{employee.employee_name} has been rejected.",
        "success"
    )
...
@app.get(
    "/founder/employee/<int:employee_id>"
)
@founder_required
